	              ...
	              TCHAR *filename, bool mylocale)
{
 ...
 int len = 2000;
	...
	if(!file)
	{
		MessageBox(NULL,TEXT("Please, check input file."),TEXT("Error") ,MB_OK|MB_ICONWARNING);
		free(buffer);
		return;		
	}
	//{		
	...
	//		//HANDLE H_IN_FILE = CreateFile(filename,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	//		//long handle                = _open_osfhandle((LONG)H_IN_FILE,0);
	//		//file = _fdopen(handle,"r");
	//		fopen_s(&file,filename,"r+");
	//		if(!file)
	//		{
	//			MessageBox(NULL,"Please, check CORRECT input file.","Error" ,MB_OK|MB_ICONWARNING);
	//			return;
	//		}
	//	}
	//	else
	 ...
	 if( quan_elen+1 == len)
		{
		 ...
		 //realloc(buffer,sizeof(double)*len); 
			...
			//double *oldbuffer = buffer;     // save pointer in case realloc fails
 ...
 if(!M_X) Var = 0;
	...
	else             Me =(vrtx.x[(vrtx.lengh/2)+1]+vrtx.x[vrtx.lengh/2])/2;
...
//void DrawCoord(HDC hdc,myData* vrtx,bool cPFlag)
//{
//	if(!vrtx->lengh)
//		return;
//	//--
...
int InsertListViewItem(HWND hwndListView, int lLin, int lCol, int iSubItemYesNo, TCHAR* txt)
{
	...
	{
		case 0:
			// Send LVM_INSERTITEM message if you want to insert an item (returns -1 if it fails):
			SendMessage((HWND) hwndListView, (UINT) LVM_INSERTITEM, (WPARAM) 0, (LPARAM) &item);
			break;
		default:
			// Send LVM_SETITEM message if you want to insert a subitem (returns FALSE if it fails):
