...
void  Calculate (HWND hList, HWND hListCh, myData &vrtx, double *num_ch, 
	               TCHAR *filename, bool mylocale)
{
	...
	if( mylocale ) setlocale (LC_NUMERIC, "Russian_Russia.1251");
	...
	while( _ftscanf (file, _T("%lf"), &buffer[quan_elen]) == 1 )
	{
		...
		if( vrtx.max_x < buffer[quan_elen] )
			   vrtx.max_x = buffer[quan_elen];
		if( vrtx.min_x > buffer[quan_elen] )
		...
		if( quan_elen+1 == len )
		{
			...
			//--
			// // MSDN предлагает нам следующий код:
			// double *oldbuffer = buffer; // save pointer, in case of realloc fails
			// if( !(buffer = (double*) realloc (buffer, sizeof(double)*len)) )
			// {
	...
	double h;
	if( quan_elen == 1 ) h = 0;
	else h = (vrtx.max_x - vrtx.min_x)/ (sqrt ( (double)quan_elen) - 1);

	if( !h)
		vrtx.length = 1;
	else
		vrtx.length = (int ) (sqrt ( (double)quan_elen));

	vrtx.n    = (int   *) malloc (sizeof (int   ) * vrtx.length);
	vrtx.x    = (double*) malloc (sizeof (double) * vrtx.length);
	vrtx.EV   = (double*) malloc (sizeof (double) * vrtx.length);
	vrtx.a    = (double*) malloc (sizeof (double) *(vrtx.length+1));
	double *V = (double*) malloc (sizeof (double) * vrtx.length);

	int i=0, j=0;
	vrtx.a[0] = vrtx.min_x - h/2;
	for(i=0; (i < vrtx.length) && (vrtx.max_x >= vrtx.a[i]); i++)
		vrtx.a[i+1] = vrtx.a[i] + h;

	vrtx.x[0] = (vrtx.a[0] + vrtx.a[1])/2;
	for(i=0; i<vrtx.length-1; i++)
		vrtx.x[i+1] = vrtx.x[i] + h;

	for(i=0; i<vrtx.length; i++)
	{
		vrtx.n[i] = 0;
		for(j=0; j<quan_elen; j++)
			if( vrtx.a[i] <= buffer[j] && buffer[j] <= vrtx.a[i+1] )
				vrtx.n[i]++;
		V[i] = (double)vrtx.n[i]/quan_elen;
	}

	double M_X = 0, M_X2 = 0, M_X3 = 0, M_X4 = 0;
	for(i=0; i<vrtx.length; i++)
	{
	...
	double	S = sqrt (S2);
	double Var = (S/M_X) * 100; // %
	if( !M_X ) Var = 0;

	double Me;
	if( (vrtx.length%2) )
		Me =  vrtx.x[ (vrtx.length)/2 ];
	else
		Me = (vrtx.x[ (vrtx.length/2) + 1] + vrtx.x[vrtx.length/2])/2;

	// мода-- 
	int  *index = (int*)calloc (vrtx.length, sizeof (int));
	int   mode_length=0;

	vrtx.max_n = vrtx.n[0];
	vrtx.min_n = vrtx.n[0];
	for(j=0; j<vrtx.length; j++)
	{
	...
	for(j=0; j<vrtx.length; j++)
		...
		double *M0 = (double*) malloc (sizeof (double)*mode_length);
		for(j=mode_length; j--; )
			M0[j] = vrtx.x[index[j]];
		//--
		double S3 = S2 * S;
		...
		double Mu3 = M_X3 - 3 * M_X2 * M_X + 2 * pow (M_X, 3);
		double Mu4 = M_X4 + 6 * sqr (M_X) * M_X2 - 3 * pow (M_X, 4) - 4 * M_X3 * M_X;
		...
		TCHAR string_to_list[100];
		double EV = 0;
		for(i=0; i<vrtx.length; i++)
		{
			EV += V[i];
			vrtx.EV[i] = EV;
			_ftprintf (file, _T("%3d [%.6lf, %.6lf) %.6lf %6d %.6lf %.6lf\n"), i + 1, vrtx.a[i], \
				vrtx.a[i + 1], vrtx.x[i], vrtx.n[i], V[i], EV);

			int k = 0;
			_stprintf (string_to_list, _T("%d"), i + 1);
			InsertListViewItem (hList, i, k, k, string_to_list);
			k++;

			_stprintf (string_to_list, _T("[%lf, %lf)"), vrtx.a[i], vrtx.a[i + 1]);
			InsertListViewItem (hList, i, k, k, string_to_list);
			k++;

			_stprintf (string_to_list, _T("%lf"), vrtx.x[i]);
			InsertListViewItem (hList, i, k, k, string_to_list);
			k++;

			_stprintf (string_to_list, _T("%d"), vrtx.n[i]);
			InsertListViewItem (hList, i, k, k, string_to_list);
			k++;

			_stprintf (string_to_list, _T("%lf"), V[i]);
			InsertListViewItem (hList, i, k, k, string_to_list);
			k++;

			_stprintf (string_to_list, _T("%lf"), EV);
			InsertListViewItem (hList, i, k, k, string_to_list);
		}
		_ftprintf (file, _T("%4d1.000000\n"), quan_elen);

		_stprintf (string_to_list, _T("%d"), quan_elen);
		InsertListViewItem (hList, vrtx.length, 0, 0, NULL);
		InsertListViewItem (hList, vrtx.length, 3, 1, string_to_list);
		InsertListViewItem (hList, vrtx.length, 4, 1, _T("1.000000"));
...
Ek = %.6lf\n"), M_X, M_X2, S2, S, Var, Me, As, Ek);
		...
		_ftprintf (file, _T("Mo = "));
		for(i = mode_length; i-- ; )
		 _ftprintf (file, _T("%.6lf, "), M0[i]);
		//--
		TCHAR symbols[8][10] = { _T("ⴳ"), 
			_T("S²"), 
			_T("S"), 
			_T("Var"), 
			_T("Me"), 
		...
		TCHAR str[8][23];
		for(j = 0; j<8; j++)
			_stprintf (str[j], _T("%6.6lf"), num_ch[j]);
		//--
		...
		for(i=0; i<8; i++)
		{
			...
			if( i < 7 )
				InsertListViewItem (hListCh, i, 2, 2, str[i]);
		}
		for(i=0; i<mode_length; i++)
		{
			_stprintf (ctr, _T("%lf"), M0[i]);		
			if( i )
				InsertListViewItem (hListCh, 7 + i, 0, 0, _T(""));
...
// 		return;
// 	//--
// 	#define MinN vrtx->min_n
// 	#define MaxN vrtx->max_n
// 	#define MinX vrtx->min_x
// 	#define MaxX vrtx->max_x
// 	//--	
// 	// int qty = vrtx->length;
...
// 		LineTo   (hdc, Rect1.right - 4 - cPFlag * 50, Rect1.bottom - 17 - cPFlag * 25);						
// 		LineTo   (hdc, Rect1.right - 8 - cPFlag * 50, Rect1.bottom - 20 );						
// 		LineTo   (hdc, Rect1.right - 4 - cPFlag * 50, Rect1.bottom - 23 + cPFlag * 25);						
// 		LineTo   (hdc, Rect1.right - 13, Rect1.bottom - 20);						
// 		//--
...
// 		for(n=0; n<3; n++)
// 		{
// 			TextOut (hdc, x0 + G_Width/2 * n - 3 - 6*(lstrlen (str_X[n]) - 2),
//                  Rect1.bottom - 42 - cPFlag * 100, str_X[n], lstrlen (str_X[n]));
// 		}
// 		for(n=0; n<3; n++)
// 		{
// 			TextOut (hdc, Rect1.left + 31 + cPFlag * 50,
//                  y0 - G_Height/4 * n*2 - 9 - cPFlag * 150, str_N[n], lstrlen (str_N[n]));
// 		}
// 		//--
// 		for(n=0; n<5; n++)
// 		{
...
// 		for (int n = 0;n<3;n++)
// 		{
// 			TextOut (hdc, x0 + G_Width/2 * n - 9*(lstrlen (str_X[n]) - 2), Rect1.bottom - 42, str_X[n], lstrlen (str_X[n]));
// 		}
// 		//--
// 		for(n=1; n<6; n++)
// 		{
...
void  DrawGraphic (HDC hdc, myData *vrtx, DWORD lpColorG, DWORD lpColorP, DWORD lpColorD, bool hChk)
{
	...
	if( vrtx->length < 2 )
		return;
	//--
	bool DrawP = true, DrawG = true, 
		    DrawC = true, DrawD = true;
...
#define MaxX vrtx->max_x
	...
	int G_Width  = (int) ((Rect1.right - Rect1.left) * 0.55);
	int G_Height = (int) ((Rect1.bottom - Rect1.top) * 0.85);

	double mX = G_Width  / dX;
	double mN = G_Height / dN;
	...
	if( DrawC)
	{
		...
		TCHAR str_X[3][10], str_N[3][10];
		//--
		_stprintf (str_X[0], _T("%3.3lf"), (MinX));
		_stprintf (str_X[1], _T("%3.3lf"), (MaxX + MinX)/2);
		_stprintf (str_X[2], _T("%3.3lf"), (MaxX));
		if( !DrawD )
		{
			_itot (MinN, str_N[0], 10);
			_itot ( (MaxN + MinN)/2, str_N[1], 10);
