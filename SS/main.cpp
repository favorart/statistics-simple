 ...
 switch (messg)
 {
  ...
  case WM_PAINT:
   ...
   HBRUSH _WHITE_BRUSH = CreateSolidBrush(0xFFFFFF);
		 ...
		 //hdc1 = BeginPaint(hLabel_2/*вот тут бля hWnd было - поэтому не работало*/, &ps1);
...
//--
void OnMenu(HWND hWnd, WPARAM wParam, 
	           TCHAR *newFile, myData *vrtx, HWND hListView, bool &mylocale,DWORD *myColor)
{
 ...
 switch(uMsg) 
 {
  case WM_INITDIALOG:
   ...
   hdc = BeginPaint(hFpDlg, &ps);
			FillRect(hdc,&Rect,(HBRUSH)(HBRUSH)CreateSolidBrush(0xEAE1DF));
			EndPaint(hFpDlg,&ps);
			return TRUE;
		}
